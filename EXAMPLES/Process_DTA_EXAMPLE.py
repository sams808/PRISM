...
class TgGuiApp:
    def __init__(self, root, path: Optional[Path] = None):
        import tkinter as tk
    ...
    def load_file(self, path: Path):
        ...
        # Initialize Tg window from x range
        if self.df is not None and xdef in self.df.columns:
            x = pd.to_numeric(self.df[xdef], errors="coerce").to_numpy()
            x = x[np.isfinite(x)]
            if len(x) > 0:
                # Always initialize Tg window to 350–700 (user convention)
