Dataapp 2.0.0 — the completed PySide6/Qt rebuild of the multi-technique spectra suite.
...
- **Project files** (`.dataapp`, format v3): the whole session — spectra, fit models, XAS store with history, HT-XRD series, CIF overlays, baseline settings — survives closing the app. A demo project ships in `EXAMPLES/demo_project.dataapp`.
...
## Getting started
- **Portable (no Python needed)**: download `Dataapp-portable.zip` below, extract, double-click `Dataapp\Dataapp.exe`. Note: Larch-dependent XAS steps (normalization/EXAFS) are not included in the portable build — use the Python route for those.
- **From a Python 3.11 install**: clone, `pip install -r requirements.txt`, double-click `Dataapp.bat` (or `python qt_main.py`).
