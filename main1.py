# =====================================================================
# 1. Imports and Global Settings  —  V40
# =====================================================================
...
# CUSTOM IMPORTS
from io_universal import load_any as load_any, import_xy as import_xy, list_columns as list_columns
...
def _auto_pick_xy(df: pd.DataFrame, meta: Dict) -> Tuple[str, str, str, bool]:
    ...
    Strategy:
      - Use parser-specific canonical_map when possible (io_universal provides it).
    ...
    if parser == "ta_sdt":
        cx = _guess_col(columns, patterns=("temperature",))
        cy = _guess_col(columns, patterns=("heat", "flow"))
        if cx and cy:
            return cx, cy, "Heuristic: Temperature vs Heat Flow", False
        cx = _guess_col(columns, patterns=("time",))
        cy = _guess_col(columns, patterns=("mass", "weight"))
        if cx and cy:
            return cx, cy, "Heuristic: Time vs Mass", False

    if parser == "saxs_edf_ascii":
        cx = _guess_col(columns, patterns=("q(", "q_"," q "))
        cy = _guess_col(columns, patterns=("i(q)", "intensity", " i "))
        if cx and cy:
...
def pick_default_xy_for_ta_sdt(df: pd.DataFrame):
    ...
    Priority Y: mass_pct → mass_mg → heatflow_mW → dT_C → dT_uV → first two numeric cols
    Returns (x, y, info_dict)
    """
    x_candidates = ["temp_C", "time_min"]
    y_candidates = ["mass_pct", "mass_mg", "heatflow_mW", "dT_C", "dT_uV"]
...
class RamanApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Raman processing")
        self.root.geometry("650x380")
        self.file_paths = []
        self.file_titles = []
        self.file_statuses = []  # "imported", "sum", "baseline"
        self.fit_param_memory = {}
    ...
    def _setup_layout(self):
        ...
        ttk.Button(frame_left, text="Import files", width=18, command=self.import_files).pack(pady=4)
    ...
    def import_files(self):
        """Open file dialog, add new files to the file list (ignore duplicates)."""
        ...
        if added_any:
            ...
            messagebox.showinfo("Import", "No new files were added.")
    ...
    def baseline_param(self):
        ...
        for fp in self.file_paths:
            try:
                payload = load_file_as_xy(fp, parent=self.root, force_popup=False)
                x, y = payload.x, payload.y
                if payload.kind == "ta_sdt":
                    # Option: prefer TA default XY if you want to override
                    try:
                        x, y, _ = pick_default_xy_for_ta_sdt(payload.df)
                    except Exception:
                        pass
                spectra_xy_list.append((x, y))
            except Exception as e:
                messagebox.showerror("Import error", f"The file cannot be read as a baseline or spectrum:\n{e}")
                spectra_xy_list.append((np.zeros(2), np.zeros(2)))
    ...
    def sum_spectra(self):
        ...
        if not self.file_paths:
            ...
            return
        SpectralSumWindow(self.root, self.file_paths, self.file_titles, callback=self.on_new_file_exported)

    def one_fit(self):
        """Open the single-spectrum fit window (SingleFitWindow)."""
        if not self.file_paths:
            messagebox.showinfo("Info", "No files to fit.")
            return
        SingleFitWindow(self.root, self.file_paths, self.file_titles, self.fit_param_memory)
    ...
    def simple_plot(self):
        """Open the SimplePlotWindow for current files."""
        if not self.file_paths:
            messagebox.showinfo("Info", "No files to plot.")
            return
        SimplePlotWindow(self.root, self.file_paths, self.file_titles)
...
class SpectralSumWindow(tk.Toplevel):
    ...
    def __init__(self, master, file_paths, file_titles, callback=None):
        super().__init__(master)
        self.title("Spectral sum")
        self.geometry("1200x750")
        self.file_paths = file_paths
        self.file_titles = file_titles
        # Load spectra (x,y) quietly
        self.spectra = []
        for fp in file_paths:
            try:
                payload = load_file_as_xy(fp, parent=self, force_popup=False)
                x, y = payload.x, payload.y
                self.spectra.append((x, y))
            except Exception:
                self.spectra.append((np.array([]), np.array([])))
    ...
    def update(self):
        ...
        if spectra_proc:
            ...
            if self.var_norm.get():
                area = _safe_trapz(y_sum, x_ref)
                if abs(area) > 1e-10:
                    y_sum = y_sum / area * 100
                else:
                    messagebox.showwarning("Normalisation", f"Norm area is very small (={area:.2e}). Normalisation skipped.", parent=self)
...
class SingleFitWindow(tk.Toplevel):
    ...
    def __init__(self, master, file_paths, file_titles, fit_param_memory):
        super().__init__(master)
    ...
    def get_xy(self):
        """
        Load spectrum and apply:
          - plot window (xmin/xmax) for visualization,
          - normalization (area=100) if requested.
        """
        payload = load_file_as_xy(self.get_current_spec_path(), parent=self, force_popup=False)
        x, y = payload.x, payload.y

        # Plot window (view only)
        try:
        ...
        # Normalization (robust to tiny areas)
    ...
    def build_lmfit_parameters(self, params_struct):
        ...
        for i, d in enumerate(params_struct):
            ...
            # ---- FWHM ----
            lmin = max(1e-9, float(d["fwhm_min"]))
            lmax = float(d["fwhm_max"])
            lval = float(d["fwhm_val"])
            if lmin > lmax:
                lmin, lmax = lmax, lmin
...
def main():
    ...
    filemenu.add_command(label="Import files…", command=app.import_files)
    ...
    root.bind("<Control-o>", lambda e: app.import_files())
